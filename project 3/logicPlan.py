...
def findModelUnderstandingCheck() -> Dict[Expr, bool]:
    ...
    a = Expr('A')
    "*** BEGIN YOUR CODE HERE ***"
    a.op = a.op.lower()  # Convert op to lowercase
    result = {(a, True)}  # Assuming the result should contain the expression and True
    return dict(result)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    myTemp = premise & ~conclusion
    return findModel(myTemp) is False
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def plTrueInverse(assignments: Dict[Expr, bool], inverse_statement: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    negated_inverse = ~inverse_statement
    return pl_true(negated_inverse, assignments)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    return PropSymbolExpr(pacman_str, x, y, time=now) % disjoin([cause for cause in possible_causes])
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
# Generate sentence for Pacman's exact position at timestep t
    pospositions = [PropSymbolExpr(pacman_str, x, y, time=t) for (x, y) in non_outer_wall_coords]
    pacphysics_sentences.append(exactlyOne(pospositions))

# Generate sentence for Pacman's exact action at timestep t
    posactions = [PropSymbolExpr(action, time=t) for action in DIRECTIONS]
...
# Add successor axioms if provided and timestep > 0
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    "*** BEGIN YOUR CODE HERE ***"
    KB.extend([
    pacphysicsAxioms(0, all_coords, non_outer_wall_coords, walls_grid, sensorModel=None, successorAxioms=allLegalSuccessorAxioms),
    pacphysicsAxioms(1, all_coords, non_outer_wall_coords, walls_grid, sensorModel=None, successorAxioms=allLegalSuccessorAxioms)
    ])

# Add initial Pacman position at timestep 0
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time=0))

# Add initial action at timestep 0
    KB.append(PropSymbolExpr(action0, time=0))

# Add action at timestep 1
    KB.append(PropSymbolExpr(action1, time=1))

# Find models with Pacman at (x1, y1) at timestep 1 and without Pacman at (x1, y1) at timestep 1
    pacmanAtTarget = findModel(conjoin(KB + [PropSymbolExpr(pacman_str, x1, y1, time=1)]))
    pacmanNotAtTarget = findModel(conjoin(KB + [~PropSymbolExpr(pacman_str, x1, y1, time=1)]))

    return (pacmanAtTarget, pacmanNotAtTarget)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        print(t)
    # Generate Pacman locations for current timestep t
        locations = [PropSymbolExpr(pacman_str, x, y, time=t) for (x, y) in non_wall_coords]
    # Ensure exactly one Pacman location for timestep t
        KB.append(exactlyOne(locations))
    
    # Check if Pacman is at the goal location at timestep t
        model = findModel(conjoin([PropSymbolExpr(pacman_str, xg, yg, time=t)] + KB))
        if model:
            return extractActionSequence(model, actions)
    
    # Generate actions for timestep t
        actions_expr = [PropSymbolExpr(action, time=t) for action in actions]
    # Ensure exactly one action for timestep t
        KB.append(exactlyOne(actions_expr))
    
    # Generate successor axioms for timestep t+1
...
# Loop over timesteps
    ...
    # Generate food variables at time t
        foodlist = [PropSymbolExpr(food_str, x, y, time=t) for (x, y) in food]
    ...
    # Generate actions for timestep t
        act = [PropSymbolExpr(action, time=t) for action in actions]
    # Ensure exactly one action for timestep t
        KB.append(exactlyOne(act))

    # Generate successor axioms for Pacman and food for timestep t+1
        for (x, y) in non_wall_coords:
    ...
    # Check if goal is reached (no food left)
        goal = ~disjoin(foodlist)
    # Check if there's a model where the goal is satisfied
        model = findModel(conjoin([goal] + KB))
        if model:
        # If a model is found, extract and return the action sequence
            return extractActionSequence(model, actions)
    "*** END YOUR CODE HERE ***"
...
def localization(problem, agent) -> Generator:
    ...
    for coord in walls_list:
        KB.append(PropSymbolExpr(wall_str, *coord))
    
    # Specify where walls are not, for all coordinates
    for coord in all_coords:
        if coord not in walls_list:  # No wall at this coordinate
            KB.append(~PropSymbolExpr(wall_str, *coord))
    ...
    for t in range(agent.num_timesteps):
        ...
        # Record action at the current timestep
        KB.append(PropSymbolExpr(agent.actions[t], time=t))
        # Process percepts and add resultant rules
        percepts = agent.getPercepts()
        KB.append(fourBitPerceptRules(t=t, percepts=percepts))
        
        possible_locations = []
        # Check for possible and provable pacman locations
        for coord in non_outer_wall_coords:
            pacman_at_coord = PropSymbolExpr(pacman_str, *coord, time=t)
            if findModel(conjoin([conjoin(KB), pacman_at_coord])):
                possible_locations.append(coord)
            if entails(conjoin(KB), pacman_at_coord):
                KB.append(pacman_at_coord)
            elif entails(conjoin(KB), ~pacman_at_coord):
                KB.append(~pacman_at_coord)
        
        # Move agent to the next state based on the current action
...
def mapping(problem, agent) -> Generator:
    ...
    known_map[pac_x_0][pac_y_0] = known_map[pac_x_0][pac_y_0]
    KB.append(PropSymbolExpr(wall_str, pac_x_0, pac_y_0) if known_map[pac_x_0][pac_y_0] == 1 else ~PropSymbolExpr(wall_str, pac_x_0, pac_y_0))
...
def slam(problem, agent) -> Generator:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    util.raiseNotDefined()
# Deleted this it was annoying me
