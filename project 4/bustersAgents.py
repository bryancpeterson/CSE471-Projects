...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        livingGhostsLikelyPositions = []
        for distribution in livingGhostPositionDistributions:
            mostLikelyPosition = distribution.argMax()
            livingGhostsLikelyPositions.append(mostLikelyPosition)

    # Determine the closest ghost to Pacman
        distancesToGhosts = []
        for ghostPosition in livingGhostsLikelyPositions:
            distance = self.distancer.getDistance(pacmanPosition, ghostPosition)
            distancesToGhosts.append((distance, ghostPosition))
        _, closestGhost = min(distancesToGhosts)
    ...
    # Choose the action that minimizes the distance to the closest ghost
        ...
        for action in legal:
            ...
            distancesForActions[action] = distance
        chosenAction = min(distancesForActions, key=distancesForActions.get)
