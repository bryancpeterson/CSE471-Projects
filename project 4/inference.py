...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # Correctly calculate the maximum possible observation value
    max_distance = X_RANGE + Y_RANGE - 2 + MAX_NOISE  # Adjusting formula to include maximum noise

    observations = list(range(max_distance + 1))  # +1 because range is exclusive at the upper bound
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        # Initialize factors list with all Conditional Probability Tables
        ...
        for variable_to_eliminate in eliminationOrder:
            # Join factors containing the current elimination variable
            current_factors, joined_factor = joinFactorsByVariable(current_factors, variable_to_eliminate)
            
            # Check if the joined factor has more than one unconditioned variable
            if len(joined_factor.unconditionedVariables()) > 1:
                # Eliminate the current variable from the joined factor
                eliminated_factor = eliminate(joined_factor, variable_to_eliminate)
                # If the elimination results in a valid factor, add it to the current factors list
                if eliminated_factor:
                    current_factors.append(eliminated_factor)

        # Join all factors to obtain the final joint distribution
        final_joint_factor = joinFactors(current_factors)

        # Normalize the joint distribution
        normalized_distribution = normalize(final_joint_factor)
...
class DiscreteDistribution(dict):
    ...
    # Check if the total sum is non-zero to avoid division by zero error
        if total_sum != 0:
        # Create a list of keys to iterate over to avoid 'dictionary changed size during iteration' error
            keys_list = list(self.keys())
        
        # Iterate over each key to update values in-place
            for current_key in keys_list:
                self[current_key] = self[current_key] / total_sum
        #raiseNotDefined()
        "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    # Check if the Ghost is in jail
        ...
        # If noisy distance is None, return probability of 1.0, else return probability of 0.0
            return 1.0 if noisyDistance is None else 0.0
    
    # Check if the observation is None (Ghost is in jail)
        if noisyDistance is None:
        # If Ghost is in jail, return probability of 1.0, else return probability of 0.0
            return 1.0 if ghostPosition == jailPosition else 0.0
    
    # Calculate probability based on noisy and true distances
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # Initialize a temporary dictionary to hold updated beliefs before normalization
        updated_beliefs = {}

        # Get Pacman's current position and jail position once, as they won't change during iteration
        pacman_position = gameState.getPacmanPosition()
        jail_position = self.getJailPosition()

        # Calculate and update beliefs based on observation probabilities
        for ghost_position in self.allPositions:
            observation_prob = self.getObservationProb(observation, pacman_position, ghost_position, jail_position)
            updated_beliefs[ghost_position] = self.beliefs[ghost_position] * observation_prob

        # Replace old beliefs with the updated ones
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        predicted_beliefs = DiscreteDistribution()

        # Pre-calculate belief values for each old position to avoid redundant calculations
        belief_values = {old_position: self.beliefs[old_position] for old_position in self.allPositions}

        # Iterate over all old positions to compute their contribution to new positions' beliefs
        for old_position, old_belief in belief_values.items():
            # Obtain the distribution over new positions given the old position
            new_position_distribution = self.getPositionDistribution(gameState, old_position)

            # Distribute the belief from the old position to the new positions
            for new_position, transition_prob in new_position_distribution.items():
                predicted_beliefs[new_position] += old_belief * transition_prob

        # Directly replace the current beliefs with the predicted beliefs
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        remainder_particles = self.numParticles % len(self.legalPositions)

        # Distribute particles evenly among positions
        for position in self.legalPositions:
            # Add particles_per_position particles for each position
        ...
        for i in range(remainder_particles):
            # Add one particle to each position in order
    ...
    def getBeliefDistribution(self):
        ...
        # Initialize an empty DiscreteDistribution for the belief distribution
        belief_distribution = DiscreteDistribution()

        # Convert counts to probabilities for each particle
        # The division by total_particles calculates the probability directly
        belief_distribution.update({particle: count / total_particles for particle, count in particle_counts.items()})
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Initialize an empty DiscreteDistribution for the new weight distribution
        ...
        if weight_distribution.total() > 0:
            # Resample particles from the updated weight distribution
